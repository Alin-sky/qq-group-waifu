...
var url3 = "https://uapis.cn/api/v1/misc/weather?city=";
async function apply(ctx) {
  let bots = {
    appId: "102062652",
    //appid ,必填
    secret: "42qSq8BzY0DLI2Zr"
    //secret,必填
  };
  let bot_tok = {
    token: "",
  ...
  function findRegion(name2) {
    for (const region of list) {
      if (region.name.includes(name2)) {
        return region.adcode;
      }
    }
    return null;
  }
  __name(findRegion, "findRegion");
  ...
  __name(getTimePeriod, "getTimePeriod");
  let arry = [];
  let kmr = [];
  ctx.command("天气 <message:text>").action(async ({ session }, message) => {
    if (!message) {
    ...
    if (true) {
      let formatCustomDateTime = function(input) {
        const [datePart, timePart] = input.split(" ");
        const [year, month, day] = datePart.split("-").map(Number);
        const [hour, minute] = timePart.split(":").map(Number);
        return `${month}月${day}日${hour}点${minute}分`;
      }, tem_color = function(temp) {
        if (temp <= 0) {
          return "❄️❄️❄️";
      ...
      __name(formatCustomDateTime, "formatCustomDateTime");
      __name(tem_color, "tem_color");
    ...
    async function sendMessage() {
      ...
      try {
        const messss = await ctx.http.post(
          `https://api.sgroup.qq.com/v2/groups/${session.guildId}/files`,
          mess,
          {
            headers: {
