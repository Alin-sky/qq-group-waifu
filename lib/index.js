...
var name = "qq-group-waifu";
var Config = import_koishi.Schema.object({
  days: import_koishi.Schema.number().min(1).max(14).step(1).default(3).description("用户刷新频率，超过该时间的用户会被删除（单位：天）"),
  hours: import_koishi.Schema.number().min(0).max(24).step(1).default(0).description("数据刷新时间，24小时制"),
  bot_appId: import_koishi.Schema.string(),
  meme_api: import_koishi.Schema.string()
});
...
async function apply(ctx, config) {
  ...
  const log = new import_koishi.Logger(log1);
  const random = new import_koishi.Random(() => Math.random());
  const bots_ass = {
    id: "",
    url: ""
  };
  function scheduleMidnightTask(taskFunction) {
    const now = /* @__PURE__ */ new Date();
    const nextMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 0, 0, 0);
    const delay = nextMidnight.getTime() - now.getTime();
    setTimeout(() => {
      taskFunction();
      scheduleMidnightTask(taskFunction);
    }, delay);
  }
  __name(scheduleMidnightTask, "scheduleMidnightTask");
  async function delet_wifes() {
    console.log("执行任务，当前时间：", /* @__PURE__ */ new Date());
    await ctx.database.remove("qqwaifu_db_marry", {});
    console.log(await ctx.database.get("qqwaifu_db_marry", {}));
  }
  __name(delet_wifes, "delet_wifes");
  ...
  async function save_user(session) {
    ...
    const etime = (/* @__PURE__ */ new Date()).setHours(config.hours, 0, 0, 0);
    let indata;
  ...
  __name(save_user, "save_user");
  async function ga_user(guild_users, session) {
    const etime = (/* @__PURE__ */ new Date()).setHours(config.hours, 0, 0, 0);
    for (let i2 = 0; i2 < guild_users.length; i2++) {
      const calcula = etime - guild_users[i2].timestemp;
      if (calcula >= config.days * 864e5) {
        guild_users.splice(i2, 1);
      } else if (calcula < config.days * 864e5 && calcula != 0) {
        guild_users[i2].status_u = false;
      }
    }
    const l_1 = guild_users.filter((i2) => i2.status_u == false);
    const l_2 = l_1.filter((i2) => i2.userid != session.event.user.id);
    ...
    switch (int_butt_data[0]) {
      case "meme-jiehun":
        ...
        if (int_butt_data[1] == sess.event.user.id) {
          ...
          uuuuu = int_butt_data[1];
        }
        const tutu = await create_meme(uuuuu, 1);
        sess.send(import_koishi.h.image(tutu, "image/jpg"));
        break;
      case "meme-momotou":
        let uuuu;
        if (int_butt_data[1] == sess.event.user.id) {
          ...
          uuuu = int_butt_data[1];
        }
        const tutu2 = await create_meme(uuuu, 0);
        sess.send(import_koishi.h.image(tutu2, "image/jpg"));
        break;
      case "/wife":
  ...
  async function get_user_wife(session) {
    let wife_data = await ctx.database.get("qqwaifu_db_marry", session.channelId);
  ...
  __name(get_user_wife, "get_user_wife");
  function send_md(opti, wife, session) {
    let usid;
    let uurl;
    if (wife.id2 == "bots") {
      usid = bots_ass.id;
    ...
    let mdp = {
      ...
      markdown: {
        content: "<qqbot-at-user id='" + session.event.user.id + "' />\n💓您今天的老婆群友是：\n![img #100px #100px](" + uurl + ")"
      }
    };
...
![img #100px #100px](${uurl})`
  ...
  __name(send_md, "send_md");
  async function create_meme(userid, type) {
    let utext;
    let json_opt = {};
    switch (type) {
      case 0:
  ...
  __name(create_meme, "create_meme");
  ctx.command("离婚").action(async ({ session }) => {
    const no_user_md = {
      msg_type: 2,
    ...
    session.send(import_koishi.h.image(tutu2, "image/jpg"));
    return;
  });
  let ii = 0;
  ctx.command("wife").option("notat", "-n 不@对方").option("console", "-c").action(async ({ session, options }) => {
    ...
    if (wifes == false) {
      ...
      indx_udata_u1 = user_data.guilds.findIndex((i2) => wifes.userid == i2.userid);
      indx_udata_u2 = user_data.guilds.findIndex((i2) => session.event.user.id == i2.userid);
    }
